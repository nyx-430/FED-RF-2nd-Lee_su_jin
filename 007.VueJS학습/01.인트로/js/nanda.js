...
/****************************************************** 
    ...
    1. 뷰인스턴스 초기화 완료 단계: created
    -> 이 단계 활용법: 데이터 셋팅을 많이 한다
    -> 사용법:
        created: function(){코드}
        created: ()=>{코드}
        created(){코드}

    2. 뷰랜더링 완료 단계: mounted
    -> 이 단계 활용법: JS, 제이쿼리 등 DOM에 그려진 후
    코딩해야 하는 부분을 여기에 연결한다!
    -> 사용법:
        mounted: function(){코드}
        mounted: ()=>{코드}
        mounted(){코드}
...
// 상품정보 생성을 위한 클래스
// 클래스는 객체 템플릿이다!
// 따라서 클래스 생성자 함수를 호출하면 객체 인스턴스가 생성된다!
