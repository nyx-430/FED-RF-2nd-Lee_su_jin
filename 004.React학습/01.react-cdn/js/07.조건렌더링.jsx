...
function Developer({ isNormal, isrc, ialt, itit }) {
  ...
  if (isNormal) {
    // 위의 if문에 걸리면 아래쪽 return에는 내려오지 않는다!
    // 리턴 코드 만들기
    return (
      <React.Fragment>
        <LostDeveloper />
...
/**************************************************** 
  4. 조건연산자(삼항연산자)를 사용하여 조건부 렌더링하기
  비?집:놀이동산
****************************************************/
// 명화 데이터
const workUrl = {
...
function ShowLikePainter({ isChange }) {
  ...
  return (
    <React.Fragment>
      ...
      {/* 3. 작품 출력 */}
      {
        // 삼항연산자로 isChange값에 따라 출력 변경하기
        // isChange가 아닌 상태 관리 변수를 사용한다!
        result ? (
...
/********************************************************* 
  ...
  2. 구현방법:
      ...
      4) 사용결과
          ...
          자동으로 변경될 때마다 다시 갱신되는 것을 확인 할 수 있다!
