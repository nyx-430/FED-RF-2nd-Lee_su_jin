...
/********************************************************** 
    ...
    5. 컴포넌트로 보내기 위해서는 HTML속성과 동일한 구문 사용
**********************************************************/
...
function IntroCar({ brand, modelNum }) {
  ...
  // props는 컴포넌트 호출시 속성값을 객체로 전달하는 것을 받아주는 변수다
  // 변수명은 자유롭게 쓸 수 있음!
  // 사용법: props.속성명
  // 주의: 속성명은 컴포넌트를 호출하는 곳에서 정해진 이름을 똑같이 사용해야 한다
  // 명시적으로 변수명을 사용하려면 구조분해 할당형식을 사용하면 된다
  // -> {변수명,변수명,...}
  // 구조분해 할당으로 받을 경우의 장점은 특정 속석명을 지정하는 효과가 있고,
...
function DetailCarInfo({ brand, model, color, opt }) {
  ...
  // 전달 속성은 model(모델명), color(색상), opt(css옵션)
...
ReactDOM.render(
  <div>
    ...
    <ShowBrandCar brand="현대제네시스" modelNum={3} />
  </div>,
  mFn.qs("#root1")
);
