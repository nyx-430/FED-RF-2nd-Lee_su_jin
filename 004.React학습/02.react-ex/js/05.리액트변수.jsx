...
/******************************************************** 
☑️ useRef : 리액트 HOOK 참조변수
1. 특정 DOM 선택할때 주로 쓰이며 .current 프로퍼티로 전달된 
인자로 초기화된 변경 가능한 ref 객체를 반환한다.

-> 예) <참조할요소/컴포넌트 ref="useRef사용변수명" .../>
-> ref속성에 값으로 설정된 useRef변수를 써준다!
이변수에 담긴 요소를 사용할 수 있다! -> 변수명.current
-> current를 써야하는 이유는 useRef컴포넌트 객체에서
이 변수를 관리해 주고 값을 유지, 업데이트 하는데 사용하는
속성으로 current를 사용하도록 지정함!

2. 그밖에 컴포넌트가 리랜더링 되어도 변경되어서는
안 되는 변수의 값이 있다면 useRef를 사용하여 일정값을 유지함!
...
// [4] 컴포넌트 외부변수
// (1) 전역변수 역할을 하지만 컴포넌트가 여러 개 사용될 때
// 이 변수를 업데이트하면 여러 가지 프로그래밍이 꼬일 수 있음
// 그러므로 절대! 절대! 절대! 변경되는 변수를 바깥 쪽에 쓰지 않는다!
// -> 리액트 철칙!!!
...
function MainComp() {
  ...
  // [1] 이름 상태관리 변수
  // (1) 변경된 값은 컴포넌트 소멸 전까지 유지됨
  // (2) 변수값 변경시 리랜더링 됨
  // (3) 값 변경시 반영은 useEffect 구역에서 확인됨!
  // -> 의미 : 실제 돔에 반영할 때 변경값이 업데이트 되는 시점임!
  const [stsName, setStsName] = React.useState("공유");

  // [2] 컴포넌트 내부 일반변수
  // -> 컴포넌트가 리랜더링 될 때 다시 초기화됨
  let varName = "김수현";
