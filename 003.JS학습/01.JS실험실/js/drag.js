...
function goDrag(ele,coll) {
  // ele - 호출시 보내준 대상을 받는 변수
  // -> HTMl 컬렉션으로 보낸 대상만 받게 한다!
  // col - 드래그 요소 전체 컬렉션을 받는 변수
  // -> 마우스 다운시 z-index 대상 1로 만들 때 다른 요소는 0
  // 다른 요소는 0 변경시 사용
  ...
  const dMove = (e) => {
    ...
    if (dragSts) {
      ...
      // 3. 이동 차를 구한 resultX, resultY 값을 대상 위치값에 적용
      // 대상 : 드래그 요소 dtg
    ...
    // 드래그 중일 때는(dragSts===true) 주먹손(grabbing),
    // 드래그 아닐 땐(dragSts===false) 편손(grab)
  ...
  // -> 왜 필요하지? 이동 후 결과 위치를 저장하여 다음 드래그 이동시
