...
// -> 일반 함수와의 차이점은 개별적인 객체 인스턴스로 함수가
...
export default function autoScrollFn() {
  ...
  // body - overflow: hidden; (스크롤바 없앰)
  document.querySelector("body").style.overflow = "hidden";

  /****************************************************** 
    [ 자동 스크롤 기능 정의 ]
    1. 스크롤바가 없는 상태에서 또는 스크롤 기능을 
    막아놓은 상태에서 마우스 휠 작동시 아래와 같이 기능 구현됨
    (1) 휠 내림 : 다음 페이지로 이동
    (2) 휠 올림 : 이전 페이지로 이동

    2. 스크롤바 첫 페이지와 끝 페이지에서 이동 안 함
  ...
  ***************************************/
  ...
  function wheelFn(e) {
    ...
    e.preventDefault();
    // -> passive:false 설정해야 함! 왜? window니까!

    // 2. 광휠 금지 장치 //////
    if (stsWheel) return; // 돌아가!
    stsWheel = true; // 잠금!
    setTimeout(() => {
      stsWheel = false; // 잠금 해제!
    }, 500);
  ...
  *********************************************************/
  ...
  function touchStartFn(e) {
    ...
    console.log("터치시 시작 위치:", mPosStart);
  } ////////// touchStartFn 함수 //////////

  // 2-2. 터치 끝 이벤트 호출 함수
  function touchEndFn(e) {
    // 1. Y축 터치 위치 알아오기
    mPosStart = e.changedTouches[0].screenY;
    // 모바일 이벤트값 객체는 touches[0]임!
    // 그러나 같은 이벤트가 연속될 경우 변경된 값을
    // 읽어와야하므로 changedTouches[0]를 사용해야 함!

    // 2. 처음 터치 위치와 마지막 위치의 차 구하기
    let diffValue = mPosEnd - mPosStart;
    // 음수가 아랫 방향 이동으로 맞추기 위해 (끝값 - 시작값)

    console.log("터치시 끝 위치:", mPosEnd, "\n차이수:", diffValue);

    // 분석 결과 :
    // 양수는 위에서 아래로 쓸어내림(아래 페이지로 이동)
    // 음수는 아래에서 위로 쓸어올림(위 페이지로 이동)
