...
2. 이때 미니언즈 버튼마다 한번에 들어가는 갯수가 정해져 있다.(1개, 5개, 10개)
...
[3] 파란 박스 리셋하기
1. 리셋 버튼을 클릭하면 파란 박스에 미니언즈 이미지 모두가 삭제된다.
...
function insertMini() {
  ...
  // getAttibute(속성명) -> 속성값 읽어오기 내장함수
  // setAttribute(속성명,값) -> 속성값 넣기 내장함수

  // 1. 함수 호출 확인
  console.log("미니언즈 들어와", num);

  // 2. 변경 대상 선정: .mini-space -> Bcase의 변수
...
function resetMini(){
    ...
    // 2. 변경 대상: .mini-space -> Bcase의 변수
    // 3. 변경 내용: 내부 태그 모두 삭제
    Bcase.innerHTML = "";
    // innerHTML의 빈 문자값을 할당하여 지운다!

    // 4. 추가 변경 대상: .stxt span -> stxt 변수
    // 5. 추가 변경 내용: 0으로 초기화
    stxt.innerText = 0;

    // 6. 이미지 개수 저장 전역벽수 0으로 초기화
    count = 0;
